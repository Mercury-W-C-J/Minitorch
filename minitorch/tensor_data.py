...
def broadcast_index(
    ...
    for i in range(diff, len(big_shape)):
        if shape[i - diff] == 1:
            out_index[i - diff] = 0
        else:
            out_index[i - diff] = big_index[i]
...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    idx = 0
    while idx < len(shape1):
        if shape1[idx] == 1:
            res = res + tuple([shape2[idx]])
            idx += 1
        else:
            if shape2[idx] == 1 or shape2[idx] == shape1[idx]:
                res = res + tuple([shape1[idx]])
                idx += 1
            else:
                raise IndexingError
    return res
